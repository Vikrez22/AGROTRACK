    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      snapshot.docs.forEach((doc) => {
        ...
        if (data.type === "grazing")
          grazing.push({ id: doc.id, coords: data.coordinates });
        else if (data.type === "non-grazing")
          nonGrazing.push({ id: doc.id, coords: data.coordinates });
      });
      ...
      >
        ...
        {userLocation && (
          <Marker position={userLocation}>
            <Popup>
              {userRole} Location: {userLocation[0].toFixed(4)},{" "}
        ...
        {cowMarkers.map((cow, idx) => {
          ...
          return (
            <Marker key={idx} position={[cow.lat, cow.lon]}>
              <Popup>
                Cow {idx + 1} (Herder): {cow.lat.toFixed(4)},{" "}
                {cow.lon.toFixed(4)}
              </Popup>
            </Marker>
          );
